        ...
        if (cur > MAX_ORDER) {
            ...
            for (int i = 0; i < arena_count; i++) {
                ...
                if (cur <= MAX_ORDER) {
                    ...
                    data->arena = arena; // Bind to original arena

                    if (pthread_setspecific(thread_cache_key, data) != 0) {
                        block->used = 0;
                        block->magic = MAGIC_FREE;
                        uint32_t o = block->order;
                        o = coalesce_up(other_arena, block, o);
